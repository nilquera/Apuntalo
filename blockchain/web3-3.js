...
class call_blockchain {
    ...
    async send_coin(sender, reciever, amount) {
        try{
            var data = await this.contract.methods.sendCoin(sender, reciever, amount).send({from: cuenta1});
            console.log(data);
            this.update();
            return data;
        } catch (error) {
    ...
    async activate_account(account) {
        try {  
            var data = await this.contract.methods.activateAccount(account).send({from: cuenta1});
            console.log(data);
            this.update();
            return data;
        } catch (error) {
...
console.log("Acción: Activar cuenta 2 y enviar x monedas de cuenta 1 a cuenta 2");
